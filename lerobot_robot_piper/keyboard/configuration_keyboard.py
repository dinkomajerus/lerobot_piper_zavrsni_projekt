...
@dataclass
class KeyboardJointTeleopConfig(TeleoperatorConfig):
    """Configuration for keyboard joint teleoperator.
    
    Used for controlling robot joints directly with keyboard inputs.
    
    Attributes:
        joint_step: Degrees to move per keypress for regular joints
        gripper_step: Degrees to move per keypress for the gripper
        use_gripper: Whether to include gripper control in actions
    """
    joint_step: float = 2.0
    gripper_step: float = 15.0
    use_gripper: bool = True
